...
int main()
{
  ...
  int num_outputs = od*oy*ox;
  int num_inputs = id*ix*iy;
  int num_weights = id*od*k*k;
  int num_biases = od;
  // float input_array[b*num_inputs + num_biases + num_weights + 100000];
  // cout << "num outputs " << num_outputs <<   endl;
  float input_array[b*num_inputs+num_biases + num_weights + b*num_outputs + num_outputs + oy*ox + ox] = {255};
  cout << "Size of array is " << b*num_inputs+num_biases + num_weights + b*num_outputs + num_outputs + oy*ox + ox <<  endl;
  cout << "Starting Test with " << numBatches << " batches" <<  endl;
...
//    // Run Accelerator
//    #ifdef HW_TEST
//    cout << "Hardware Version " << endl;
//    hw_conv_layer(HW_CTRL_ADDR, input_array, 0,
//                  sizeof(float)*(b*num_inputs+num_biases + num_weights),
//                  b, od, ox, oy, id, ix, iy, s, k);
//
//    #else
  conv_layer(input_array, 0, sizeof(float)*(b*num_inputs+num_biases + num_weights),
              b, od, ox, oy, id, ix, iy, s, k);
//    #endif
